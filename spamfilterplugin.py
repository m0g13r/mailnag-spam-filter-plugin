...
class SpamfilterPlugin(Plugin):
    ...
    def get_config_ui(self):
        ...
        spin_t.set_tooltip_text(_("Limit for blocking: If an email's total weight reaches this value, it's filtered. Lower = stricter."))
        ...
        def add_row(parent, label_text, value_name, tv_name, help_txt, def_val=0, is_bonus=False, height=80):
            ...
            if value_name:
                header.pack_start(Gtk.Label(label=_("Bonus (Score -):") if is_bonus else _("Weight (Score +):"), xalign=1), False, False, 0)
        ...
        add_row(tab_rules, "Spam Keywords", "weight_kw", "kw", _("How many points to add for each found keyword. Doubles for subject matches."), 2)
        add_row(tab_rules, "Regex Patterns", "weight_rx", "rx", _("Points added for each technical pattern match. Doubles for subject matches."), 3)
        add_row(tab_rules, "Blocked TLDs", "weight_tl", "tl", _("Points added if the sender uses one of these domains (e.g., .xyz)."), 5, height=50)
        tab_sources = add_tab(_("Sender Lists"))
        add_row(tab_sources, "Whitelist (Always Allow)", None, "wl", _("Trusted emails/domains that are always accepted without any scoring."), height=60)
        add_row(tab_sources, "Infra Spam (Always Block)", None, "infra", _("Known spammer server patterns that are blocked immediately without scoring."), height=60)
        add_row(tab_sources, "Trusted Sources (Bonus)", "bonus_tr", "tr", _("Safe senders that reduce the total spam score by these points."), 4, True, height=60)
        ...
        add_row(tab_brands, "Protected Brands", None, "br", _("Checks if a mail claims to be from a brand but uses a fake domain. Adds Regex Weight."), height=150)
